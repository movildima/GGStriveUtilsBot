...
namespace GGStriveUtilsBot.Utils
{
    static class DustloopDataFetcher
    {
        ...
        const int maxResults = 3; //will play around with this to see how bad it gets
        ...
        public static MoveListInternal fetchMove(string character, string move)
        {
            ...
            {
                if (dataMove.input.ToLower() == move.ToLower())
                    results1.Add(dataMove);
                else if (Levenshtein.Distance(dataMove.name.ToLower(), move.ToLower()) < LDistance)
                    results1.Add(dataMove);
            }
            ...
            //swap the image parameter with an actual image link
            foreach(var dataMove in results2)
            {
                var response = new WebClient().DownloadString(String.Format(imgQuery, dataMove.images[0]));
                var t = JObject.Parse(response).SelectToken("query.pages.*.imageinfo[0].url").Value<string>(); //I hate this
                dataMove.images[0] = t;
            }
